#!/usr/bin/env bash
# Validate the cluster and record exactly the facts experiment protocol Task 01
# requires (uname -a, os-release, kubelet/kubectl/containerd versions,
# cgroup v2 mount, CONFIG_BPF_LSM, active LSM list) plus general
# node/pod/CNI status, into $RUN_DIR/validation for environment.json/.md.
set -euo pipefail
...
kubectl --kubeconfig "$KUBECONFIG_PATH" version > "$OUT_DIR/kubectl-version.out" 2>&1 || true
...
for entry in "control-plane:$CONTROL_PLANE_PUBLIC_IP" "worker:$WORKER_PUBLIC_IP"; do
  ...
  remote_capture "$host" "${role}-kubeadm-version" "kubeadm version"
  remote_capture "$host" "${role}-cgroup-mount" "mount | grep cgroup"
  remote_capture "$host" "${role}-cgroup-controllers" "cat /sys/fs/cgroup/cgroup.controllers 2>/dev/null || true"
  remote_capture "$host" "${role}-bpf-lsm-config" "grep CONFIG_BPF_LSM /boot/config-\$(uname -r) 2>/dev/null || echo 'CONFIG_BPF_LSM=<not present in /boot/config>'"
  remote_capture "$host" "${role}-active-lsm" "cat /sys/kernel/security/lsm 2>/dev/null || echo '<securityfs lsm list not present>'"
  remote_capture "$host" "${role}-bpftool-path" "command -v bpftool && bpftool version || true"
done

cat > "$OUT_DIR/summary.txt" <<SUMMARY
Task 01 Azure CPU campaign (cpu-campaign-20260813) validation artifacts
...
- kernel/os-release/containerd/kubelet/cgroup/BPF-LSM: ${OUT_DIR}/*.{out,err}
SUMMARY
